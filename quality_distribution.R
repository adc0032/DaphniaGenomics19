...
dat <- data.frame(stringsAsFactors = CHROM, POS, QUAL)
dat.qual <- select(dat, QUAL)

library(ggplot2)
ggplot(dat.qual) + geom_histogram()

ggsave(quality.distribution, plot = last_plot(), device = "pdf", limitsize = TRUE)
